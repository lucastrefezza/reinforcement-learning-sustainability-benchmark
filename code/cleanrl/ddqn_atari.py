    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    # Use the main Q-network to select actions
                    next_q_values = q_network(data.next_observations)
                    best_actions = next_q_values.argmax(dim=1) # best_actions contains the indexes

                    # Evaluate the Q-value of the best actions using the target network:
                    # first get the q-values through the target network
                    target_q_values = target_network(data.next_observations)
                    # then select the actions considered best by the q_network
                    target_max = target_q_values.gather(1, best_actions.unsqueeze(1)).squeeze(1)
