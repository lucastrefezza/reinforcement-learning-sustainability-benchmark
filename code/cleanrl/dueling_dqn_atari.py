...
class DuelingQNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.feature_learning_layers = nn.Sequential(
        ...
        self.state_value_function = nn.Linear(512, 1)
        self.advantage_function = nn.Linear(512, env.single_action_space.n)

    def forward(self, x):
        features = self.feature_learning_layers(x / 255.0)
        values = self.state_value_function(features)
        advantages = self.advantage_function(features)
        # dim=1 let us calculate the mean for each row (representing a state),
        # i.e. the mean advantage over each action of that state
